...
# Defiene unparallelized version
```
...
if(run_==T){
  ...
  store = store %>% mutate(price_poachers_bertrand1 = s_b_own(ss_bertrand1, a_f, a_w, b_f, b_w, e, W, c),
    ...
    write.csv(store, here('data', 'sensitivity_steady_states.csv'), row.names = F)
...
# Loop over several runs is faster than 1 big run
  ...
  store = read.csv(here('data','sensitivity_steady_states.csv'))
}
