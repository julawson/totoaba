...
result1 <- as.data.frame(read.csv(here("data","result1.csv")) %>% select(x, growth) %>% rename(value=x))
...
a <- a %>% 
  filter(spec=="c and v") %>% 
  filter(str_detect(output, "ss_")) %>% 
  select(value, output, spec, transformation) 
a$value <- as.integer(a$value)


merged <- full_join(result1, a)
merged2 <- merged %>% drop_na()

ggplot() +
  geom_line(data=merged, aes(x = value, y = growth)) +
  geom_point(data=merged2, aes(x = value, y = growth, color=transformation))

#Maybe select the points and create plots showing the variability in steady states by a given parameter (x axis = spec; y axis = values)

ggplot() +
  geom_point(data=merged2, aes(x=transformation, y=value, color=spec))
...
new_c = data.frame(c=c(3*c,3*c), v=c(v,v), value = c(1, 1))
...
p + 
  ...
  geom_point(data = init_eq, col = 'black')+
...
df_cv %>%
  ...
  scale_fill_gradient2(low = low, 
                       mid = mid,
                       high = high)+
  facet_grid(vars(gamma),vars(output))+
  theme_bw()+
  theme(legend.position = 'bottom')+
  guides(colour = guide_legend(title.position = "top"))
...
## Idea for other analysis

### 1. Need to check when things backfire i.e when we can have ss_bertrand1 < ss_monop1
...
### 2. Understand the evolution of the steady states with respect to the other variables
