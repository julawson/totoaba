...
```{r load sens analysis file}

sens <- read.csv(here('data', 'sensitivity_steady_states.csv'))

sens_plot <- sens %>% 
  group_by(gamma_primitive) %>% 
  summarize(mean_X = mean(X, na.rm=T),
            mean_X1 = mean(X.1, na.rm=T),
            mean_X2 = mean(X.2, na.rm=T),
            mean_X3 = mean(X.3, na.rm=T),
            mean_X4 = mean(X.4, na.rm=T),
            mean_X5 = mean(X.5, na.rm=T),
            mean_X6 = mean(X.6, na.rm=T),
            mean_X7 = mean(X.7, na.rm=T),
            mean_X8 = mean(X.8, na.rm=T))

ggplot(sens_plot, aes(x=gamma_primitive)) +
  geom_line(aes(y=mean_X), linetype = "dashed")+
  geom_line(aes(y = mean_X1))+
  geom_line(aes(y = mean_X2))+
  geom_line(aes(y = mean_X3))+
  geom_line(aes(y = mean_X4))+
  geom_line(aes(y = mean_X5))+
  geom_line(aes(y = mean_X6))+
  geom_line(aes(y = mean_X7))+
  geom_line(aes(y = mean_X8))+
...
# which choice is endogeneous
dat_aqua = read.csv(here("data", 
                         "aqua_par_farm.csv"))
...
gamma_sens = c(0.0001, .01, .1/.99, .2/.99, .5/.99, .8/.99, .9/.99, NA)
