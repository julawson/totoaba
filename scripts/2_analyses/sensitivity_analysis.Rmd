...
# Get a grid of all possible combinations we may want to try
...
# Include other parameters to load for parallel computing
other_params = data.frame(k, sigma, W, beta)%>%
   slice(rep(1:n(), each = nrow(params_sensitivity)))

# Finally, get all the parameter combinations we want to do the model on
...
## D. Multiprocessing run

Now, use multiprocessing to perform analysis. I define chunks and then let the code run on the chunks of data. To run the model, define `r run_ = T`, otherwise leave it as is, and load the data. 
...
# Loop over several runs is faster than 1 big run
  ...
  write.csv(store, here('data', 'sensitivity_steady_states.csv'))
}else{
  sensitivity = read.csv(here('data','sensitivity_steady_state.csv'))
}

```

## E. Evaluate steady state outcomes

```{r}
# Need to load all the parameters

# EXISTING : 
  # Get quantities for harvest with growth(x)
  # Load function for farming, apply them to x
  # Price paid to poachers

# TO MAKE : 
  # Price
  # Profit

# OUTPUT : tidy data to leverage ggplot, we're gonna have a lot of stuff to plot.
  # Data table
  # Column 1 : number
  # Column 2 : label for quantity
  # Columns 3... : parameters
