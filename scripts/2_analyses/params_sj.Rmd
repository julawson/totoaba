...
With this model, typically, observed catch in 2017 yields a price of `r price_obs` \$ USD per kilo of totoaba In this case, the value of a 25.7 kilos totoaba is `r 25.7*price_obs` \$
...
units = append(units, c('unitless', 'metric tons of biomass', 'vessel trips $^{-1}$'))
...
params = append(params, c('gamma', 'v', 'c'))
value = append(value, c(gamma, v, c))
concept = append(concept, c('Substitutability', 'unit cost of farming', 'unit costs associated with trading'))
units = append(units, c('unitless', 'USD/ton of biomass', 'USD/ton of biomass'))
...
* $c$ can move a lot I think. Our estimate is based on fines, and bribes, and convictions. It's a lot, but I'm sure there is an argument to increase it. 
