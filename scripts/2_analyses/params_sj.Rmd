...
dat_b %>% 
  ...
  geom_vline(xintercept = 1974, color = 'grey', linewidth = 1, linetype ='dashed')+
...
catchability = dat_b %>% subset(y==2016) %>% select(c)/(max_E *dat_b %>% subset(y==2016) %>% select(b))
...
```{r}
dat_sensititivy = read.csv(here('data', 'sensitivity_analysis.csv'))
