...
value = c(model$coefficients[1], - model$coefficients[2])
concept = c('Demand model : intercept', 'Demand model : coefficient')
units = c('USD/metric ton of biomass', 'USD/metric ton of biomass')
...
catchability = 0.0005526788/25.7# in vessel trips ^-1
...
```{r}
result1%>%
  mutate(diff_m_c = qty_monop - q_cournot_harvest,
         diff_m_b = qty_monop - q_bertrand_harvest,
         diff_c_b = q_cournot_harvest - q_bertrand_harvest)%>%
  ggplot(aes(x=x))+
  geom_line(aes(y = diff_m_c, colour= 'Diff monop & cournot'))+
  geom_line(aes(y = diff_m_b, colour= 'Diff monop & bertrand'))+
  geom_line(aes(y = diff_c_b, colour = 'Diff cournot & betrand'))
```