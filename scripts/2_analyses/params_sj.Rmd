...
r = 0.2012827
k = 20226 # in metric tons of biomass from INAPESCA, 2020
k_ = k*1000 # in kilos
...
to_add = (dat_c$total_cost-dat_c$cost_gear_confiscations)/dat_c$vesseldays_year

avg_cost = 8215.34 + to_add# This is average cost per trip
min_cost = 3992.27 + to_add# This is the minimum cost per trip (low season)
max_cost = 13327.92 +to_add# This is the maximum cost per trip (high season)
...
$$
CT(E_{hist}) = WE^2 \Rightarrow MC(E_{hist}) = 2WE_{hist} = \kappa \Rightarrow W = \frac{\kappa}{2E_{hist}}
$$
...
c = dat_c$cost_gear_confiscations/totoaba_ton
...
#gamma_prim = dat_aqua %>% subset(Gamma>0.8 & Gamma < 1) %>% select(Gamma) %>% pull()
#gamma = gamma_prim*beta

# Determine cost for comparable fish
age = dat_aqua %>% subset(Gamma>0.8 & Gamma < 1) %>% select(Totoaba_Age) %>% pull()
v = dat_aqua %>% subset(Gamma>0.8 & Gamma < 1) %>% select(actual_v) %>% pull()

# Set gamma to be almost perfectly substitutable
gamma = 0.9*beta
# Cost parameter for monopoly : 
params = append(params, c('age','gamma_prim','gamma', 'v', 'c'))
value = append(value, c(age,gamma/beta, gamma, v, c))
...
result1 %>%
  ...
  geom_line(aes(y = qty_monop, colour = 'Monopoly harvest'), linewidth= 1.1)+
  geom_line(aes(y = q_cournot_harvest, colour = 'Cournot harvest'), linewidth = 1.1)+
...
result1 = result1 %>% mutate(q_bertrand_harvest = q_b_own(x,v,c),
                             q_bertrand_harvest_illustration = q_b_own(x, .5*v,c),
                             q_bertrand_harvest_c_illustration = q_b_own(x,v,10*c),
...
result1 %>%
  ...
  geom_line(aes(y = qty_monop, colour = 'Monopoly harvest'), linewidth = 1.1)+
  #geom_line(aes(y = growth, colour = 'Growth'))+
  geom_line(aes(y = q_bertrand_harvest, colour = 'Betrand harvest'), linewidth = 1.1)+
...
result1 = result1 %>% mutate(ss_monop = growth - qty_monop,
                               ...
                               q_pt = ((alpha-c) * x^2 * sigma^2)/(4*W + beta * sigma^2 * x^2),
                               ss_q_pt = growth - q_pt)
steady_states = c()
  
for(variable in c('ss_monop', 'ss_cournot', 'ss_bertrand', 'ss_q_pt')){
...
# In case of monopoly : 
dat_monop = result1 %>% subset(x %in% steady_states[1:3]) %>%
  select(x, qty_monop)%>%
  mutate(p = alpha - beta*qty_monop)
kable(dat_monop)
# In case of cournot :
dat_cournot = result1 %>% subset(x %in% steady_states[4:6])%>%
  select(x, q_cournot_farmed, q_cournot_harvest)%>%
  mutate(price_paid_to_poacher = s_c_own(x))
kable(dat_cournot)
# In case of Bertrand : 
dat_bertrand = result1 %>% subset(x %in% steady_states[7:9])%>%
  select(x, q_bertrand_farmed, q_bertrand_harvest)%>%
  mutate(price_paid_to_poacher = s_b_own(x),
         diff_cost = price_paid_to_poacher + c - v)
kable(dat_bertrand)

dat_q_pt = result1 %>% subset(x %in% steady_states[10:12])%>%
  select(x, q_pt)
kable(dat_q_pt)

print(paste0('Monopoly scenario with cost parameter c=',c, ' is a ', round((dat_monop$x - dat_q_pt$x)/dat_q_pt$x*100,2), '% variation in stock in tonnes (compared to OA) or, equivalently, of ', round((dat_monop$x - dat_monop$x)*1000/25.7), ' adult totoabas'))

# Stock and harvest variation compared to baseline - Bertrand
print(paste0('Worst case scenario with cost parameter c=',c, ' is a ', round((dat_bertrand$x - dat_monop$x)/dat_monop$x*100,2), '% variation in stock in tonnes or, equivalently, of ', round((dat_bertrand$x - dat_monop$x)*1000/25.7), ' adult totoabas'))

print(paste0('Worst case scenario with cost parameter c=',c, ' is a ', (dat_bertrand$q_bertrand_harvest- dat_monop$qty_monop), 'tons in harvest in tonnes or, equivalently, of ', round((dat_bertrand$q_bertrand_harvest- dat_monop$qty_monop)*1000/25.7), ' adult totoabas'))

# Stock and harvest variation compared to baseline - Cournot

print(paste0('Best case scenario with cost parameter c=',c, ' is a ', round((dat_cournot$x - dat_monop$x)/dat_monop$x*100,2), '% variation in stock in tonnes or, equivalently, of ', round((dat_cournot$x - dat_monop$x)*1000/25.7), ' adult totoabas'))

print(paste0('Best case scenario with cost parameter c=',c, ' is a ', (dat_cournot$q_cournot_harvest- dat_monop$qty_monop), 'tons variation in harvest in tonnes or, equivalently, of ', round((dat_cournot$q_cournot_harvest- dat_monop$qty_monop)*1000/25.7), ' adult totoabas'))
...
knitr::kable(calibration, caption = 'Parameters used for calibration')
...
summary(dat_sensititivy$ss_monop1)

print('Distribution of Cournot steady states')
summary(dat_sensititivy$ss_cournot1)

print('Distribution of Betrand steady states')
summary(dat_sensititivy$ss_bertrand1)
```
...
# Simple figures for paper:

Illustrate equilibria
...
Illustration for the proof of Lemma 2, intuition : 
...
v_func = function(x,c){
  ...
  parenthesis = (2*W*b_w*(2*b_w*b_f - e^2) + (4*b_w*b_f - e^2)*sigma^2*x^2)/(2*W*b_m + 2*sigma^2*x^2)
...
tester %>%
  subset( x > 10000)%>%
  ggplot(aes(x=x))+
  geom_line(aes(y = v_func, colour = 'F(x, c=2658)'), linewidth = 1.1)+
  geom_line(aes(y = v_func2, colour = 'F(x, c=13290)'), linetype = 'dashed')+
...
result1 %>%
  subset(x < 1.5*max(actual_steady))%>%
  mutate(q_pt = ((alpha-c) * x^2 * sigma^2)/(4*W + beta * sigma^2 * x^2))%>%
  ggplot(aes(x=x))+
  geom_line(aes(y = growth, colour = 'Growth'), linewidth = 1)+
  geom_line(aes(y = qty_monop, colour = 'Monopoly'), linewidth = 1)+
  geom_line(aes(y = q_cournot_harvest, colour = 'Cournot'), linewidth = 1)+
  geom_line(aes(y = q_bertrand_harvest, colour = 'Bertrand'), linewidth = 1)+
  geom_line(aes(y = q_pt, colour = 'Price taker'), linewidth = 1)+
  # Vertical lines
  #geom_segment(x = actual_steady[1], xend = actual_steady[1], y=0, yend=growth(actual_steady[1]), linetype = 2)+
  #geom_segment(x = actual_steady[2], xend = actual_steady[2], y=0, yend=growth(actual_steady[2]), linetype = 2)+
  #geom_segment(x = actual_steady[3], xend = actual_steady[3], y=0, yend=growth(actual_steady[3]), linetype = 2)+
  ...
  theme_bw()+
  ylab('Growth / harvest')+
  scale_color_manual(values = c('red','lightgreen','grey', 'black', 'lightblue' ))+
  labs(colour=NULL)+
  theme(legend.position = 'bottom')
...
result1 %>%
  ...
  geom_line(aes(y = qty_monop, colour = 'Monopoly'), linewidth = 1)+
  geom_line(aes(y = q_cournot_harvest, colour = 'Cournot'), linewidth = 1)+
  geom_line(aes(y = q_bertrand_harvest_c_illustration, colour = "Bertrand (cost = 10c)"), linewidth = 1)+
  #geom_line(aes(y = q_pt, colour = 'Price taker'),linewidth = 1)+
  # Vertical lines
  #geom_segment(x = actual_steady[1], xend = actual_steady[1], y=0, yend=growth(actual_steady[1]), linetype = 2)+
  #geom_segment(x = actual_steady[2], xend = actual_steady[2], y=0, yend=growth(actual_steady[2]), linetype = 2)+
