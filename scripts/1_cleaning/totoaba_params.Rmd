...
c_par <- read.csv(here("data","c_par.csv")) #Cost data, from Buche Bros
...
#Adding in gear and vessel confiscation

w_pal_val_conf <- w_par_val %>%
  add_column(net_confs_cost_fleetday=664000/1920) %>% #Total cost as a proportion of reported confiscations averaged over the year (1,920 fishing trips total)
  add_column(vessel_confs_cost_fleetday=1230000/1920) %>% #Total cost as a proportion of reported vessel seizures averaged over the year (1,920 fishing trips total)
  add_column(fines_cost_fleetday=81675/1920) %>% #Total cost as a proportion of reported vessel seizures averaged over the year (1,920 fishing trips total)
  mutate(cost_per_trip = (cost_crew_earnings + cost_food_fuel + cost_gear + bribes_year)/total_fleet_days_year) %>% 
...
# Now calculating cost per trip and cost per totoaba 

c_par_val <- c_par %>%
  mutate(cost_per_trip=total_cost/vesseldays_year) %>% 
  mutate(cost_per_totoaba=total_cost/totoaba_year)
...
# The second line removes pseudo-fixed costs associated with staffing (office, hatchery, farming, leasing facility space, etc.). These costs would be somewhat fixed even if the number of cages increased (to a certain extent).
