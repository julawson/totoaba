...
P <- 4.20358 # exogenous market price per unit of output ($USD per tonne of totoaba biomass)
...
s <- 0.48550 # price paid to poachers ($USD per tonne of totoaba biomass)
...
#ggplot(s_par, aes(x=weight_kg_fresh, y=price_usd_perkg)) +
#  geom_point() +
#  geom_smooth(method="lm", se=TRUE, formula = y ~ x) 

lm(price_usd_perkg~weight_kg_fresh, data=s_par)

# For every 1-kg increase in the weight_kg_fresh, the price per kilogram increases by $3,608 USD
...
s_par <- s_par %>% 
  mutate(totoaba_weight_g = weight_g_fresh/0.0116) %>% 
  mutate(totoaba_weight_kg = totoaba_weight_g/1000) %>% 
  mutate(totoaba_price_usd_perkg = price_usd_perkg*0.0116) %>% 
  mutate(totoaba_weight_t = totoaba_weight_kg/1000) %>%
  mutate(totoaba_price_usd_pert = totoaba_price_usd_perkg/1000)

lm(totoaba_price_usd_pert~totoaba_weight_t, data=s_par)

# Final parameter for s, fishers paid 0.48550 USD per tonne of totoaba

### Now calculating P, exogenous market price per unit of output ($USD per tonne of totoaba biomass)

# Change dry weight to wet weight (conversion factor is 0.31, sources: INAPESCA 2018; True 2012; True 2018)
# Need x to be weight in tonnes of fresh totoaba
# Need y to be price per tonne of totoaba

P_par <- P_par %>% 
  mutate(weight_g_wet = weight_g_dried/0.31) %>% 
  mutate(totoaba_weight_g = weight_g_wet/0.0116) %>%
  mutate(totoaba_weight_kg = totoaba_weight_g/1000) %>%
  mutate(totoaba_weight_t = totoaba_weight_kg/1000) %>%
  mutate(price_per_kg_wet = price_per_kg_dried/0.31) %>%
  mutate(totoaba_price_usd_perkg = price_per_kg_wet*0.0116) %>% 
  mutate(totoaba_price_usd_pert = totoaba_price_usd_perkg/1000)

#ggplot(P_par, aes(x=totoaba_weight_t, y=totoaba_price_usd_pert)) +
#  geom_point() +
#  geom_smooth(method="lm", se=TRUE, formula = y ~ x) 

lm(totoaba_price_usd_pert~totoaba_weight_t, data=P_par)
