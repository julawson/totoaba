...
Generate results saved at `~/data/result1.csv`.
```{r, Results}
...
2. **Monopoly scenario** with cost parameter c= `r c` is a difference of `r round(dat_monop$Harvest - dat_q_pt$Harvest,2)`(in tons) in *harvest* or, equivalently, of `r round((dat_monop$Harvest- dat_q_pt$Harvest)*1000/25.7)` adult totoabas
...
4. **Worst case scenario** with cost parameter c=`r c` is a difference of `r round(dat_bertrand$Harvest- dat_monop$Harvest,2)` (tons) in *harvest* or, equivalently, of `r round((dat_bertrand$Harvest- dat_monop$Harvest)*1000/25.7,2)` adult totoabas
...
6. **Best case scenario** with cost parameter c=`r c` is a difference of `r round(dat_cournot$Harvest- dat_monop$Harvest,2)` (in tons) in harvest in tonnes or, equivalently, of `r  round((dat_cournot$q_cournot_harvest- dat_monop$qty_monop)*1000/25.7,2)` adult totoabas
